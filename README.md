...
The master of whisperers is starting to lose track of all the comings and goings in Westeros. So decides to build an GraphQL API to keep track.
...
We also provide a React client that is set ut to consume data exposed from out GraphQL servers. All client-related tasks are marked with "Frontend" and are not required to finish the workshop. Follow the link below to read installation instructions for the client

- [Client (React.js)](/client/README.md)

Your React.js app should now show some html on [http://localhost:3000]().
